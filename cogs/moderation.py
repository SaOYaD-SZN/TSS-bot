...
class Moderation(commands.Cog):
    ...
    async def unban(self, ctx, user_id: str):
        try:
            ...
            await ctx.send(
                embed=mod_embed(
                    f"{EMOJI['sparkle']} Unbanned", f"**{user}** can come back now."
                )
            )
        except (discord.NotFound, ValueError):
            await ctx.send(
                embed=mod_embed(
                    f"{EMOJI['cross']} Not found",
                    "Couldn't find that user in the ban list.",
                    COLORS["error"],
                )
            )
    ...
    @commands.hybrid_command(
        name="timeout", description="Timeout (mute) a member for a number of minutes."
    )
    @app_commands.describe(
        member="Member to timeout", minutes="Duration in minutes", reason="Reason"
    )
        ...
        *,
        ...
        await ctx.send(
            embed=mod_embed(
                ...
                f"**{member}** is timed out for **{minutes}**"
                f" minute(s).\n**Reason:** {reason}",
            )
        )
    ...
    async def untimeout(self, ctx, member: discord.Member):
        ...
        await ctx.send(
            embed=mod_embed(
                f"{EMOJI['sparkle']} Timeout removed", f"**{member}** can talk again."
            )
        )

    # ---------- warn ----------
    @commands.hybrid_command(
        name="warn", description="Warn a member (saved permanently)."
    )
    ...
    async def warnings(self, ctx, member: discord.Member):
        ...
        if not rows:
            await ctx.send(
                embed=mod_embed(
                    f"{EMOJI['sparkle']} Clean record", f"**{member}** has no warnings."
                )
            )
            return
        lines = [
            f"**#{r['id']}** — {r['reason']} (by <@{r['moderator_id']}>)" for r in rows
        ]
        await ctx.send(
            embed=mod_embed(f"{EMOJI['warn']} Warnings for {member}", "\n".join(lines))
        )
    ...
    async def clearwarnings(self, ctx, member: discord.Member):
        ...
        await ctx.send(
            embed=mod_embed(
                f"{EMOJI['sparkle']} Cleared",
                f"All warnings for **{member}** were cleared.",
            )
        )
    ...
    async def purge(self, ctx, amount: app_commands.Range[int, 1, 100]):
        ...
        deleted = await ctx.channel.purge(
            limit=amount + (1 if not ctx.interaction else 0)
        )
    ...
    # ---------- lock / unlock ----------
    @commands.hybrid_command(
        name="lock", description="Lock the current channel (no one can send messages)."
    )
    ...
    async def lock(self, ctx):
        ...
        await ctx.send(
            embed=mod_embed(f"{EMOJI['lock']} Locked", "This channel is now locked.")
        )
    ...
    async def unlock(self, ctx):
        ...
        await ctx.send(
            embed=mod_embed(
                f"{EMOJI['unlock']} Unlocked", "This channel is now unlocked."
            )
        )
    ...
    @commands.hybrid_command(
        name="slowmode",
        description="Set slowmode delay (seconds) for this channel. 0 to disable.",
    )
    ...
    async def slowmode(self, ctx, seconds: app_commands.Range[int, 0, 21600]):
        ...
        text = (
            "Slowmode disabled."
            if seconds == 0
            else f"Slowmode set to **{seconds}** second(s)."
        )
    ...
    @app_commands.describe(
        member="Member", nickname="New nickname (leave empty to reset)"
    )
    ...
    async def setnick(self, ctx, member: discord.Member, nickname: str | None = None):
        ...
        await ctx.send(
            embed=mod_embed(
                f"{EMOJI['sparkle']} Nickname updated",
                f"**{member}**'s nickname is now **{nickname or member.name}**.",
            )
        )
    ...
    async def removerole(self, ctx, member: discord.Member, role: discord.Role):
        ...
        await ctx.send(
            embed=mod_embed(
                f"{EMOJI['sparkle']} Role removed",
                f"Removed **{role.name}** from **{member}**.",
            )
        )
