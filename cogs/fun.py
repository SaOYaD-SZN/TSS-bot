...
class Fun(commands.Cog):
    ...
    async def eightball(self, ctx, *, question: str):
        embed = fun_embed(
            f"{EMOJI['star']} The 8-ball says...", random.choice(EIGHTBALL_ANSWERS)
        )
    ...
    async def coinflip(self, ctx):
        ...
        await ctx.send(
            embed=fun_embed(
                f"{EMOJI['coin']} Coin flip", f"It landed on... **{result}**!"
            )
        )
    ...
    async def roll(self, ctx, dice: str = "1d6"):
        try:
            ...
            await ctx.send(
                embed=fun_embed(
                    f"{EMOJI['cross']} Invalid format",
                    "Use the format NdM, like `2d6`.",
                    COLORS["error"],
                )
            )
            return
        ...
        await ctx.send(
            embed=fun_embed(
                f"{EMOJI['sparkle']} Dice roll",
                f"{', '.join(map(str, rolls))} (total: **{sum(rolls)}**)",
            )
        )
    ...
    @commands.hybrid_command(
        name="ship",
        description="Calculate compatibility between two members (just for fun!).",
    )
    @app_commands.describe(
        member1="First member", member2="Second member (defaults to you)"
    )
    async def ship(
        self, ctx, member1: discord.Member, member2: discord.Member | None = None
    ):
    ...
    async def compliment(self, ctx, member: discord.Member | None = None):
        ...
        await ctx.send(
            embed=fun_embed(f"{EMOJI['flower']} Aww!", f"{member.mention}, {text}")
        )
    ...
    async def meme(self, ctx):
        try:
            ...
            await ctx.send(
                embed=fun_embed(
                    f"{EMOJI['cross']} Oops",
                    "Couldn't fetch a meme right now, try again soon!",
                    COLORS["error"],
                )
            )
    ...
    async def joke(self, ctx):
        try:
            ...
            async with self.session.get(
                ...
                data = await resp.json()
            await ctx.send(
                embed=fun_embed(f"{EMOJI['bear']} Here's a joke!", data["joke"])
            )
        except Exception:
            await ctx.send(
                embed=fun_embed(
                    f"{EMOJI['cross']} Oops",
                    "Couldn't fetch a joke right now, try again soon!",
                    COLORS["error"],
                )
            )
    ...
    async def _send_nekos(self, ctx, endpoint, title, with_target=True, member=None):
        try:
            ...
            await ctx.send(
                embed=fun_embed(
                    f"{EMOJI['cross']} Oops",
                    "Couldn't fetch that right now, try again soon!",
                    COLORS["error"],
                )
            )
    ...
    async def cuddle(self, ctx, member: discord.Member):
        await self._send_nekos(
            ctx, "cuddle", f"{EMOJI['bear']} Cuddles!", member=member
        )
    ...
    async def trivia(self, ctx):
        try:
            ...
            options = [
                html.unescape(o) for o in q["incorrect_answers"] + [q["correct_answer"]]
            ]
            ...
            embed = fun_embed(
                f"{EMOJI['book']} Trivia time!", f"{question}\n\n{lettered}"
            )
            ...
            await ctx.send(
                embed=fun_embed(
                    f"{EMOJI['cross']} Oops",
                    "Couldn't fetch trivia right now, try again soon!",
                    COLORS["error"],
                )
            )
    ...
    # ---------- guess the number ----------
    @commands.hybrid_command(
        name="guess", description="Play a quick guess-the-number game (1-100)."
    )
    async def guess(self, ctx):
        target = random.randint(1, 100)
        await ctx.send(
            embed=fun_embed(
                f"{EMOJI['sparkle']} Guess the number!",
                "I'm thinking of a number between 1 and 100."
                " You have 6 tries — reply in chat!",
            )
        )
        ...
        for attempt in range(6):
            try:
                ...
                await ctx.send(
                    embed=fun_embed(
                        f"{EMOJI['clock']} Time's up",
                        f"The number was **{target}**. Try again sometime!",
                    )
                )
                return
            ...
            await ctx.send(
                embed=fun_embed("Close!", f"Try {hint}! ({5 - attempt} tries left)")
            )
        await ctx.send(
            embed=fun_embed(
                "Out of tries!", f"The number was **{target}**. Better luck next time!"
            )
        )
