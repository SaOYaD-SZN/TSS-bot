...
class Study(commands.Cog):
    ...
    # ---------- pomodoro ----------
    @commands.hybrid_command(name="pomodoro", description="Start a pomodoro focus timer (default 25 min focus / 5 min break).")
    @app_commands.describe(focus_minutes="Focus duration in minutes", break_minutes="Break duration in minutes")
    async def pomodoro(self, ctx, focus_minutes: app_commands.Range[int, 1, 180] = 25, break_minutes: app_commands.Range[int, 1, 60] = 5):
        await ctx.send(embed=study_embed(
            f"{EMOJI['book']} Focus time started!",
            f"Focus for **{focus_minutes}** minutes. I'll let you know when it's break time! {EMOJI['sparkle']}",
        ))
        await asyncio.sleep(focus_minutes * 60)
        await ctx.send(embed=study_embed(
            f"{EMOJI['clock']} Break time!",
            f"{ctx.author.mention} nice work! Take a **{break_minutes}** minute break. {EMOJI['flower']}",
        ))
    ...
    # ---------- timer ----------
    @commands.hybrid_command(name="studytimer", description="Set a plain countdown timer, e.g. for a quick study sprint.")
    @app_commands.describe(minutes="Minutes to count down")
    async def studytimer(self, ctx, minutes: app_commands.Range[int, 1, 300]):
        await ctx.send(embed=study_embed(f"{EMOJI['clock']} Timer started", f"I'll ping you in **{minutes}** minute(s)!"))
        await asyncio.sleep(minutes * 60)
        await ctx.send(embed=study_embed(f"{EMOJI['sparkle']} Time's up!", f"{ctx.author.mention} your timer is done!"))

    # ---------- study tip ----------
    @commands.hybrid_command(name="studytip", description="Get a random study tip.")
    async def studytip(self, ctx):
        await ctx.send(embed=study_embed(f"{EMOJI['book']} Study tip", random.choice(STUDY_TIPS)))

    # ---------- streaks ----------
    @commands.hybrid_command(name="checkin", description="Check in for today to build your study streak.")
    async def checkin(self, ctx):
        ...
        one_day = 86400
        if now - last < one_day:
            await ctx.send(embed=study_embed(f"{EMOJI['cross']} Already checked in", "You've already checked in today — come back tomorrow!", COLORS["error"]))
            return
        if now - last < 2 * one_day:
            new_streak = row["current_streak"] + 1
        else:
            new_streak = 1
        longest = max(row["longest_streak"], new_streak)
        await self.bot.db.update_streak(ctx.author.id, new_streak, longest, now, row["total_sessions"] + 1)
        await ctx.send(embed=study_embed(
            f"{EMOJI['star']} Checked in!",
            f"Current streak: **{new_streak}** day(s) 🔥\nLongest streak: **{longest}** day(s)\nTotal check-ins: **{row['total_sessions'] + 1}**",
        ))

    @commands.hybrid_command(name="streak", description="View your current study streak.")
    ...
    # ---------- todo list ----------
    @commands.hybrid_command(name="todo", description="Add a task to your personal study to-do list.")
    ...
    async def todo(self, ctx, *, task: str):
        ...
        await ctx.send(embed=study_embed(f"{EMOJI['check']} Added", f"Added to your to-do list: *{task}*"))

    @commands.hybrid_command(name="todolist", description="View your personal to-do list.")
    async def todolist(self, ctx):
        rows = await self.bot.db.list_todos(ctx.author.id)
        if not rows:
            await ctx.send(embed=study_embed(f"{EMOJI['book']} To-do list", "Your list is empty! Add one with `/todo`."))
            return
        lines = []
        for r in rows:
            mark = "✅" if r["done"] else "🩷"
            lines.append(f"{mark} **#{r['id']}** {r['task']}")
        await ctx.send(embed=study_embed(f"{EMOJI['book']} Your to-do list", "\n".join(lines)))
    ...
    async def tododone(self, ctx, todo_id: int):
        ...
        await ctx.send(embed=study_embed(f"{EMOJI['check']} Nice work!", f"Marked #{todo_id} as done. 🎉"))
    ...
    async def tododelete(self, ctx, todo_id: int):
        ...
        await ctx.send(embed=study_embed(f"{EMOJI['sparkle']} Removed", f"Deleted #{todo_id} from your list."))
