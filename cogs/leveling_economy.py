...
class Leveling(commands.Cog):
            ...
            with contextlib.suppress(discord.HTTPException):
                await message.channel.send(
                    embed=econ_embed(
                        ...
                        f" **Level {new_level}**! {EMOJI['sparkle']}",
                    )
                )

    # ---------- rank ----------
    @commands.hybrid_command(
        name="rank", description="Show your (or someone's) level and XP."
    )
    ...
    # ---------- leaderboard ----------
    @commands.hybrid_command(
        name="leaderboard", description="Show the server's top XP earners."
    )
    async def leaderboard(self, ctx):
        ...
        await ctx.send(
            embed=econ_embed(f"{EMOJI['star']} Leaderboard", "\n".join(lines))
        )

    # ---------- balance ----------
    @commands.hybrid_command(
        name="balance", description="Check your (or someone's) coin balance."
    )
    ...
    async def daily(self, ctx):
        ...
        await ctx.send(
            embed=econ_embed(
                ...
                f" See you tomorrow! {EMOJI['sparkle']}",
            )
        )
    ...
    # ---------- pay ----------
    @commands.hybrid_command(
        name="pay", description="Give some of your coins to another member."
    )
    @app_commands.describe(member="Who to pay", amount="How many coins")
    async def pay(
        self, ctx, member: discord.Member, amount: app_commands.Range[int, 1, None]
    ):
        if member.id == ctx.author.id:
            await ctx.send(
                embed=econ_embed(
                    f"{EMOJI['cross']} Nice try",
        ...
        if row["balance"] < amount:
            await ctx.send(
                embed=econ_embed(
                    f"{EMOJI['cross']} Not enough coins",
        ...
        await ctx.send(
            embed=econ_embed(
                f"{EMOJI['coin']} Sent!",
                f"**{ctx.author.display_name}** sent **{amount}** coins"
    ...
    async def shop(self, ctx):
        ...
        if not items:
            await ctx.send(
                embed=econ_embed(
                    f"{EMOJI['shop']} Shop", "The shop is empty right now."
                )
            )
            return
        lines = [
            f"**#{i['id']} — {i['name']}**"
            f" · {i['price']} {EMOJI['coin']}\n{i['description'] or ''}"
            for i in items
        ]
        await ctx.send(embed=econ_embed(f"{EMOJI['shop']} Shop", "\n\n".join(lines)))

    @commands.hybrid_command(
        name="additem", description="(Admin) Add an item to the shop."
    )
    @app_commands.describe(
        name="Item name", price="Price in coins", description="Item description"
    )
    ...
    async def additem(
        ...
        await ctx.send(
            embed=econ_embed(
                f"{EMOJI['sparkle']} Item added",
                f"**{name}** added to the shop for **{price}** coins.",
            )
        )

    @commands.hybrid_command(
        name="removeitem", description="(Admin) Remove an item from the shop."
    )
    ...
    async def removeitem(self, ctx, item_id: int):
        ...
        await ctx.send(
            embed=econ_embed(
                f"{EMOJI['sparkle']} Removed",
                f"Item **#{item_id}** removed from the shop.",
            )
        )
    ...
    async def inventory(self, ctx):
        ...
        if not rows:
            await ctx.send(
                embed=econ_embed(
                    f"{EMOJI['shop']} Inventory", "You don't own anything yet."
                )
            )
            return
        lines = [f"**{r['name']}** x{r['quantity']}" for r in rows]
        await ctx.send(
            embed=econ_embed(f"{EMOJI['shop']} Your inventory", "\n".join(lines))
        )

    # ---------- admin xp/coin adjust ----------
    @commands.hybrid_command(
        name="addcoins", description="(Admin) Give coins to a member."
    )
