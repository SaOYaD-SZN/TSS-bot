...
def render_template(template: str, member: discord.Member) -> str:
    return (
        (template or "Welcome {member} to {guild}! 🎀")
...
class TicketPanelView(discord.ui.View):
    ...
    @discord.ui.button(
        ...
        emoji="🎫",
        custom_id="tss:open_ticket",
    )
    async def open_ticket(
        self, interaction: discord.Interaction, button: discord.ui.Button
    ):
        bot: typing.Any = interaction.client
        ...
        if not cfg or not cfg["support_role_id"]:
            await interaction.response.send_message(
                embed=wt_embed(
                    f"{EMOJI['cross']} Not configured",
                    "Tickets aren't set up yet. Ask an admin to run `/ticketsetup`.",
                    COLORS["error"],
                ),
                ephemeral=True,
            )
        ...
        overwrites: dict[
            ...
            typing.cast(discord.Member, interaction.user): discord.PermissionOverwrite(
        ...
        await interaction.response.send_message(
            embed=wt_embed(
                f"{EMOJI['sparkle']} Ticket created", f"Your ticket: {channel.mention}"
            ),
            ephemeral=True,
        )
...
class CloseTicketView(discord.ui.View):
    ...
    async def close_ticket(
        ...
        await interaction.response.send_message(
            embed=wt_embed(
                f"{EMOJI['lock']} Closing...", "This ticket will close in 5 seconds."
            )
        )
...
class WelcomeTickets(commands.Cog):
    ...
    async def on_member_join(self, member: discord.Member):
        ...
        if not isinstance(ch, discord.TextChannel):
            return
        embed = wt_embed(
            f"{EMOJI['flower']} Welcome!",
            render_template(cfg["welcome_message"], member),
        )
    ...
    async def on_member_remove(self, member: discord.Member):
        ...
        if not cfg or not cfg["leave_channel_id"]:
            return
        ch = member.guild.get_channel(cfg["leave_channel_id"])
        if not isinstance(ch, discord.TextChannel):
            return
        embed = wt_embed(
            f"{EMOJI['cross']} Goodbye",
            render_template(
                cfg["leave_message"] or "{member} has left {guild}.", member
            ),
        )
    ...
    @commands.hybrid_command(
        name="setwelcome", description="(Admin) Set the welcome channel and message."
    )
    @app_commands.describe(
        channel="Channel to post welcomes in",
        message="Use {member} and {guild} as placeholders",
    )
    ...
    async def setwelcome(
        ...
        *,
        message: str = "Welcome {member} to {guild}! 🎀",
    ):
        await self.bot.db.set_welcome(ctx.guild.id, channel.id, message)
        await ctx.send(
            embed=wt_embed(
                f"{EMOJI['sparkle']} Saved",
                f"Welcome messages will be posted in {channel.mention}.",
            )
        )

    @commands.hybrid_command(
        name="setleave", description="(Admin) Set the leave channel and message."
    )
    @app_commands.describe(
    ...
    async def setleave(
        ...
        channel: discord.TextChannel,
        *,
        message: str = "{member} has left {guild}.",
    ):
        await self.bot.db.set_leave(ctx.guild.id, channel.id, message)
        await ctx.send(
            embed=wt_embed(
                f"{EMOJI['sparkle']} Saved",
                f"Leave messages will be posted in {channel.mention}.",
            )
        )

    @commands.hybrid_command(
        name="testwelcome", description="(Admin) Preview the welcome message."
    )
    @commands.has_permissions(manage_guild=True)
    async def testwelcome(self, ctx):
        ...
        if not cfg or not cfg["welcome_message"]:
            await ctx.send(
                embed=wt_embed(
                    f"{EMOJI['cross']} Not set",
                    "Run `/setwelcome` first.",
                    COLORS["error"],
                )
            )
            return
        await ctx.send(
            embed=wt_embed(
                f"{EMOJI['flower']} Preview",
                render_template(cfg["welcome_message"], ctx.author),
            )
        )

    # ---------- ticket config ----------
    @commands.hybrid_command(
        name="ticketsetup", description="(Admin) Configure the ticket system."
    )
    @app_commands.describe(
    ...
    async def ticketsetup(
        ...
        await ctx.send(
            embed=wt_embed(
                f"{EMOJI['sparkle']} Configured",
                "Ticket system is ready! Use `/ticketpanel` to post the button.",
            )
        )

    @commands.hybrid_command(
        name="ticketpanel",
        description="(Admin) Post the 'Open Ticket' button in this channel.",
    )
    @commands.has_permissions(manage_guild=True)
    async def ticketpanel(self, ctx):
        embed = wt_embed(
            f"{EMOJI['ticket']} Need help?",
            "Click the button below to open a private support ticket! 🎫",
        )
        await ctx.send(embed=embed, view=TicketPanelView())

    @commands.hybrid_command(
        name="addtoticket", description="Add a member to the current ticket."
    )
    @app_commands.describe(member="Member to add")
    async def addtoticket(self, ctx, member: discord.Member):
        ch = ctx.channel
        assert isinstance(ch, discord.TextChannel)
        ticket = await self.bot.db.get_ticket(ch.id)
        if not ticket:
            await ctx.send(
                embed=wt_embed(
                    f"{EMOJI['cross']} Not a ticket",
        ...
        await ctx.channel.set_permissions(
            discord.Object(id=member.id), view_channel=True, send_messages=True
        )
        await ctx.send(
            embed=wt_embed(
                f"{EMOJI['sparkle']} Added",
                f"{member.mention} can now see this ticket.",
            )
        )

    @commands.hybrid_command(
        name="closeticket", description="Close the current ticket."
    )
    async def closeticket(self, ctx):
        ...
        if not ticket:
            ...
            return
        await self.bot.db.close_ticket(ch.id)
        await ctx.send(
            embed=wt_embed(
                f"{EMOJI['lock']} Closing...", "This ticket will close in 5 seconds."
            )
